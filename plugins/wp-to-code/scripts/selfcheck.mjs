...
const runDiff = (cwd) =>
  new Promise((resolve) => {
    const child = spawn(process.execPath, [join(HERE, 'diff.mjs'), '--page', 'fixture'], {
      cwd,
      env: process.env,
    });
    let out = '';
...
async function scenario(name, originalHtml, portHtml) {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      viewports: [1440],
      pages: [
        { slug: 'fixture', targetRoute: '/', originalRoot: 'main', portRoot: 'main' },
      ],
    }),
  );
...
assert.equal((same.out.match(/MATCH/g) ?? []).length, 3, 'all three sections should match');

const shifted = await scenario('same heights, shifted position', SECTIONS, SHIFTED);
